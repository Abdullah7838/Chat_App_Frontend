...
//           {/* Display messages */}
//           <div className="overflow-y-auto max-h-96 mb-4 scroll-container scrollbar-hover">
//             {messages.map((message, index) => (
//               <div
//                 key={index}
//                 className={`mt-2 p-2 rounded-md ${message.sender === userId
...
export default function Chat() {
  ...
  const sendText = async () => {
    if(msg.length >50){
      alert('Message Length should be less than 50 words');
      return;
    }
    if (msg.trim() === "") return;
  ...
  return (
    <div className="bg-black min-h-screen p-4">
      ...
      <div className="text-center text-white text-2xl font-semibold mb-4">Chat Room</div>
      <div className="text-center text-blue-500 text-xl font-bold mb-4">Your ID: {userId}</div>
      <div className="text-center text-green-500 text-lg mb-4">People Online: {onlineCount}</div>
        ...
        <div className="w-full max-w-4xl bg-blue-400 p-4 rounded-lg shadow-lg">
          ...
          <div className="overflow-y-auto max-h-96 mb-4 scroll-container scrollbar-hover">
            {messages.map((message, index) => (
              ...
              >
                {/* Display the user ID above each message in small text */}
                <div className="text-xs text-zinc-300 mb-1"><strong>id:</strong> {message.sender}</div>
